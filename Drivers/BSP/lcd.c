...
void lcd_print_plot(uint32_t * vol_plot_array,uint32_t arraylen, uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,uint16_t color)
{
    float d_x;
    float d_y;
    float i=0;
    ...
    //起始位置
    float y0 = (y1+y2)/2;
    for(i=0,c=0;c<arraylen-1;i+=d_x,c++){
        lcd_draw_line(x1+i,y0 + vol_plot_array[c]*d_y, x1+i+d_x , y0 + vol_plot_array[c+1]*d_y ,color);
    }
}